...
void LCD_ShowFloat3(u16 x,u16 y,float num,u8 len,u8 size)//最多只能显示到123.45
{ 
	      int a;
	      a=num*100;
				if(a<0)
				...
				LCD_ShowxNum(x+size/2,y,a/100,3,size,0);
				
				LCD_ShowChar(x+size/2*4,y,'.',size,0);
								
				LCD_ShowxNum(x+size/2*5,y,(a-(a/100)*100)/10,1,size,0);
				
				LCD_ShowxNum(x+size/2*6,y,(a-(a/100)*100-((a-(a/100)*100)/10)*10),1,size,0);
}


void LCD_ShowFloat4(u16 x,u16 y,float num,u8 len,u8 size)
{ 
	      int a;
	      a=num*100;
				if(a<0)
				...
				LCD_ShowxNum(x+size/2,y,a/100,4,size,0);//若是想显示1234.56的话，就是4
				
				LCD_ShowChar(x+size/2*5,y,'.',size,0);//分别乘5 6 7
								
				LCD_ShowxNum(x+size/2*6,y,(a-(a/100)*100)/10,1,size,0);
				
				LCD_ShowxNum(x+size/2*7,y,(a-(a/100)*100-((a-(a/100)*100)/10)*10),1,size,0);
}
...
{      
    ...
    for(t=0;t<32;t++) //=16*16/8 ¼ÆËã×Ö½ÚÊý
    {   
       temp=myHZ16[index*32+t]; 
       for(t1=0;t1<8;t1++)
       {
		if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
