...
/**
 * @brief  MLT_BT05 串口接收回调函数
 * @retval None
 **/
void RxCallback(){
	UART_HandleTypeDef*  huart = ble.huart; //MLT_BT05用到的串口
	...
	if (__HAL_UART_GET_FLAG(huart, UART_FLAG_IDLE) != RESET) {
		...
		int recv_end    = RxBufferSize - __HAL_DMA_GET_COUNTER(ble.dma_rx); // 得到接收到的字符长度
		int recv_start  = recv_end % RxBufferSize;                      // 接收到的内容地起始索引
		while(ble.RxBuffer[recv_start] == 0) { //找出接收的起始
		...
		ble.AckBuffer[i] = 0; //用作字符串分隔符

		//调试用，将接收到的响应通过串口输出
		print(&huart3,"\r\n接收:\r\n");
		print(&huart3,"%s",ble.AckBuffer);
		//使用队列来接收
		if(ble.uart_state == Sent) {	//如果串口发送完毕，等待接收
			ble.uart_state = Idle; 	//变为空闲模式
		}
		if(ble.uart_state == Idle) { //（一般是被动地接收到信息）
		//for(;;); //TODO:
...
void setTimeout(uint16_t ms){
	__HAL_TIM_SET_AUTORELOAD(ble.tim, ms*3/2);  //要设置延迟10ms的中断，则需要设置 ARR 为 10*3/2;
	__HAL_TIM_SET_COUNTER(ble.tim, 1);          //因为要根据CNT是否为0来判断是否超时，所以这里设置为1
	__HAL_TIM_ENABLE(ble.tim);                  //开启定时器，定时器在溢出后自动归0，并停止运作
}
...
/**
 * @brief  MLT_BT05 初始化函数
 * @retval 初始化是否成功
 **/
result init(){
	bleInit();
	UART_HandleTypeDef*  huart = ble.huart; //ESP8266用到的串口
	/* 开启DMA接收 */
	HAL_UART_Receive_DMA(huart, (uint8_t*)ble.RxBuffer, RxBufferSize);
	...
	HAL_TIM_OnePulse_Init(ble.tim, TIM_OPMODE_SINGLE);                 //设置为 OnePulse 模式
	__HAL_TIM_SET_PRESCALER(ble.tim, HAL_RCC_GetHCLKFreq()/3/1000-1);  //设置分频数为 56,000
