...
{
	if(screen_id==0 && control_id==6){				//扫频输出开启
		LED0 = !LED0;
		sweepFrequencyOn();
	}
	if(screen_id==0 && control_id== 24){			//扫频输出关闭
		LED0 = !LED0;
		sweepFrequencyOff();
	}
	if(screen_id==0 && control_id==22){				//单频输出
		LED0 = !LED0;
	}
}
...
{ 
	if(screen_id == 0 && control_id == 1){		//修改单频输出频率
		step = atol((char*)str);
		LED0 = !LED0;
	}
	if(screen_id == 0 && control_id == 13){		//修改单频输出幅值
		amplitude = atol((char*)str);
		LED0 = !LED0;
	}
	if(screen_id == 0 && control_id == 25){		//修改扫频输出幅值
		amplitude = atol((char*)str);
		LED0 = !LED0;
	}
}
